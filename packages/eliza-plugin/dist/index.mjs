...
// src/schemas.ts
import { z } from "zod";
var pubkeySchema = z.string().trim().min(32).max(44).regex(/^[1-9A-HJ-NP-Za-km-z]+$/, "Must be a valid base58 Solana public key");
var u64Schema = z.union([
  z.string().regex(/^\d+$/, "Must be a non-negative integer string"),
  z.number().int().nonnegative()
]).transform((v) => BigInt(v));
var slippageBpsSchema = z.number().int().min(0).max(1e4).default(50);
var swapSchema = z.object({
...
var addLiquiditySchema = z.object({
...
var removeLiquiditySchema = z.object({
...
var addLiquidityAction = {
  ...
  handler: async (runtime, _message, _state, options, callback) => {
    ...
    try {
      const info = await client.poolInfo(new PublicKey2(mintA), new PublicKey2(mintB));
...
Depositing \u2026`
    ...
    try {
      const params = {
        mintA: new PublicKey2(mintA),
        mintB: new PublicKey2(mintB),
        amountA,
        amountB,
        autoCompound,
...
var removeLiquidityAction = {
  ...
  handler: async (runtime, _message, _state, options, callback) => {
    ...
    try {
      const info = await client.poolInfo(new PublicKey3(mintA), new PublicKey3(mintB));
      if (info.lpSupply > 0n) {
...
Withdrawing \u2026`
    ...
    try {
      const params = {
        mintA: new PublicKey3(mintA),
        mintB: new PublicKey3(mintB),
        lpShares,
        minA: minA ?? 0n,
        minB: minB ?? 0n
      };
...
var poolInfoAction = {
  ...
  handler: async (runtime, _message, _state, options, callback) => {
    ...
    try {
      ...
      const info = await client.poolInfo(new PublicKey4(mintA), new PublicKey4(mintB));
