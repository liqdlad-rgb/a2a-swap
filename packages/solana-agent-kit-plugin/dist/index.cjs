...
__export(index_exports, {
  addLiquidityAction: () => addLiquidity_default,
  buildClient: () => buildClient,
  capabilityCardAction: () => capabilityCard_default,
  default: () => index_default,
  extractSigner: () => extractSigner,
  poolInfoAction: () => poolInfo_default,
  removeLiquidityAction: () => removeLiquidity_default,
  solscanTx: () => solscanTx,
  swapAction: () => swap_default
});
...
function extractSigner(agent) {
  ...
  if (payer instanceof import_web3.Keypair) return payer;
  throw new Error(
...
var pubkeySchema = import_zod.z.string().refine(
  (s) => {
    try {
      new import_web32.PublicKey(s);
      return true;
    } catch {
      return false;
    }
...
var swapAction = {
  ...
  schema: import_zod.z.object({
    ...
    slippageBps: import_zod.z.number().int().min(0).max(1e4).default(50).describe("Max acceptable slippage in basis points. 50 = 0.5%. Default: 50.")
  }),
  handler: async (agent, input) => {
    const mintIn = new import_web32.PublicKey(input.inputMint);
    const mintOut = new import_web32.PublicKey(input.outputMint);
...
var pubkeySchema2 = import_zod2.z.string().refine(
  (s) => {
    try {
      new import_web33.PublicKey(s);
      return true;
    } catch {
      return false;
    }
...
var u64Schema = import_zod2.z.union([
  import_zod2.z.string().regex(/^\d+$/, "Must be a non-negative integer string"),
  import_zod2.z.number().int().nonnegative()
...
var addLiquidityAction = {
  ...
  schema: import_zod2.z.object({
    ...
    autoCompound: import_zod2.z.boolean().default(false).describe(
  ...
  handler: async (agent, input) => {
    const mintA = new import_web33.PublicKey(input.mintA);
    const mintB = new import_web33.PublicKey(input.mintB);
...
var pubkeySchema3 = import_zod3.z.string().refine(
  (s) => {
    try {
      new import_web34.PublicKey(s);
      return true;
    } catch {
      return false;
    }
...
var u64Schema2 = import_zod3.z.union([
  import_zod3.z.string().regex(/^\d+$/, "Must be a non-negative integer string"),
  import_zod3.z.number().int().nonnegative()
]).describe("Token amount in atomic units");
var removeLiquidityAction = {
  ...
  handler: async (agent, input) => {
    const mintA = new import_web34.PublicKey(input.mintA);
    const mintB = new import_web34.PublicKey(input.mintB);
...
var pubkeySchema4 = import_zod4.z.string().refine(
  (s) => {
    try {
      new import_web35.PublicKey(s);
      return true;
    } catch {
      return false;
    }
...
var poolInfoAction = {
  ...
  handler: async (agent, input) => {
    const mintA = new import_web35.PublicKey(input.mintA);
    const mintB = new import_web35.PublicKey(input.mintB);
    const client = buildClient(agent);
    const info = await client.poolInfo(mintA, mintB);
    return {
      status: "success",
...
var capabilityCardAction = {
  ...
  schema: import_zod5.z.object({
    includeLivePoolInfo: import_zod5.z.boolean().default(false).describe(
